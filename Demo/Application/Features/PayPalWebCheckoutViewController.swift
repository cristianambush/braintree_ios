...
//    )
    ...
    @objc func tappedPayPalCheckout(_ sender: UIButton) {
        ...
        // Creates a new BTAPIClient on each button click. BTAPIClient lifecycle dies when completion handler executed.
    ...
    @objc func tappedPayPalVault(_ sender: UIButton) {
        ...
        let _paypalClient = BTPayPalClient(apiClient: api)
        
        _paypalClient.tokenize(request) { nonce, error in
            sender.isEnabled = true
